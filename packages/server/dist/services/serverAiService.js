...
const logLLMRequest = (model, systemInstruction, prompt, options) => {
    if (!isLoggingEnabled)
        return;
...
const generateContent = async (systemInstruction, prompt, selectedModel, options = {}) => {
...
export const suggestTags = async (promptContent, selectedModel) => {
...
export const refinePrompt = async (promptContent, selectedModel, options) => {
    ...
    if (options.persona ?? true)
        systemInstruction += `**1. Persona:** Assign a highly relevant and authoritative role or persona.\n`;
    if (options.task ?? true)
        systemInstruction += `**2. Task:** Clarify and decompose the primary task into specific, actionable steps.\n`;
    if (options.context ?? true)
        systemInstruction += `**3. Context:** Incorporate background information and constraints.\n`;
    if (options.format ?? true)
        systemInstruction += `**4. Format:** Specify the desired output structure and style.\n`;
...
export const suggestTitle = async (promptContent, selectedModel) => {
