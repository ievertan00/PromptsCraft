...
const getApiKeys = () => {
  ...
  if (!geminiApiKey) {
    throw new Error("VITE_GEMINI_API_KEY is not set in the .env file on the server.");
  }
  if (!deepseekApiKey) {
    throw new Error("VITE_DEEPSEEK_API_KEY is not set in the .env file on the server.");
  }
