...
const createSettingsTable = (callback) => {
            ...
            db.serialize(() => {
                ...
                db.run(`CREATE TABLE IF NOT EXISTS prompts (
                    ...
                    db.all("PRAGMA table_info(prompts)", (err, columns) => {
                        ...
                        const hasIsFavoriteColumn = columns.some((col) => col.name === 'is_favorite');
...
export const updatePromptFavoriteStatus = (promptId, isFavorite) => {
    return new Promise((resolve, reject) => {
        const db = getDB();
        db.run(`UPDATE prompts SET is_favorite = ? WHERE id = ?`, [isFavorite ? 1 : 0, promptId], function (err) {
            if (err) {
...
const handleContextColumnMigration = (callback, columns) => {
